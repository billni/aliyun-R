fun_control <- function(dir) {

gc()
starttime <- Sys.time()

file <- paste(dir, "/project4R/aliyun-stock/res/stocklist.txt", sep="")
###########################################
...
getmaincost <- function(stock, i) {
  ...
  stock[i, "主力控盘日期"] <- substr(s[1], start+3, end[[1]][2]) 
  stock[i, "主力控盘日期"]  <- gsub("年", "-", stock[i, "主力控盘日期"])
  stock[i, "主力控盘日期"]  <- gsub("月", "-", stock[i, "主力控盘日期"])
  stock[i, "主力控盘日期"]  <- gsub("日", "", stock[i, "主力控盘日期"])
    
  # main control level
  start <- regexpr("该股为" , s[1])
  end <- regexpr("盘" , s[1])
  stock[i, "控盘程度"] <- substr(s[1], start+3, end) 
